"""
Assignment-compatible test entrypoint for the MLOps pipeline project.

This file preserves the existing project tests from test_pipeline.py and
makes them runnable via the required command:
    pytest test.py -v
"""
...
class TestDataIngestion(unittest.TestCase):
    ...
    def test_train_test_split_via_main(self):
        """Run the full ingestion main() flow against a temp params.yaml."""
        cwd = os.getcwd()
        try:
...
class TestDataPreprocessing(unittest.TestCase):
    ...
    def test_clean_data_fills_categorical_na_with_mode(self):
        ...
        self.assertEqual(cleaned["cat"].iloc[-1], "yes")
